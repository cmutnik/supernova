...
%Multiple detections of the same object were required%in that night.

\begin{figure}[h!]%[H]
\begin{center}
    \includegraphics[width=1\linewidth]{figures/Starrat_mag_low_alpha0tac5_remake.png}%
     \caption{\it \small{57680 is used as an example to show the distribution of starrat as a function of magnitude.  Starrat values for all objects detected that night are shown in blue.  Shown in red are the starrat values of good stationary transient detections made that night.~\label{fig:starrat}}}%     Shown in red, good SNe were isolated after applying classification restrictions.~\label{fig:starrat}}}
  \end{center}
\end{figure}
...
%Classification variable values 
...
%
%Once light curves are constructed for each object, 
...
polynomials.  
%If a second degree polynomial has the lowest $\Chi^2$, the object is a SN.  
...
%Width and height are two parameters of a Gaussian fit that can used 
%to distinguish between different types of SN.  
